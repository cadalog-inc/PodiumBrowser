...
function updateExpiry($id) {
    $url = 'https://api.keygen.sh/v1/accounts/5c8527e9-edad-4ee6-8240-2c2b7ec16415/licenses/' . $id;
...
function activate($key, $fingerprint) {    
    ...
    if(is_null($license->body->data->attributes->expiry)) {
        $license = updateExpiry($license->body->data->id);
    }
    ...
    if($license->statusCode == 200) {
        ...
        if($statusCode == 201) {
            return (object) array(
                ...
                "id" => $machine->data->id,
                "expiry" => date_parse($license->body->data->attributes->expiry)
            );
        } else {
            return (object)array(
                "statusCode" => $statusCode,
